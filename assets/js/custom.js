...
$(document).ready(function($) {
    ...
    if( customizerEnabled == 1 ){
        $.getScript( "http://localhost/devhucp/assets/misc/customizer.js", function( data, textStatus, jqxhr ) {
...
// Render hero search form ---------------------------------------------------------------------------------------------
    ...
    $("select").on("rendered.bs.select", function () {
        $('head').append( $('<link rel="stylesheet" type="text/css">').attr('href', 'http://localhost/devhucp/assets/css/bootstrap-select.min.css') );
        if( !viewport.is('xs') ){
    ...
    if( $(".calendar").length ){
        ...
        for( var i = 1 ; i<=12 ; i++ ){
            ...
            $("#month_"+i).zabuto_calendar({
                ajax: {
                    url: "http://localhost/devhucp/assets/php/calendar.php",
                    modal: true
                },
                action: function () {
    ...
    $(".form-email .btn[type='submit']").on("click", function(){
        ...
        form.validate({
            submitHandler: function() {
                $.post("http://localhost/devhucp/assets/external/email.php", form.serialize(),  function(response) {
...
function openModal(target, modalPath){
    ...
    $("#" + target + ".modal").on("show.bs.modal", function () {
        ...
        lastModal = _this;
        $.ajax({
            url: "http://localhost/devhucp/assets/external/" + modalPath,
            method: "POST",
            //dataType: "html",
            data: { id: target },
            success: function(results){
                ...
                if( $("input[type=file]").length ){
                    $.getScript( "http://localhost/devhucp/assets/js/jQuery.MultiFile.min.js", function( data, textStatus, jqxhr ) {
                        $("input.file-upload-input").MultiFile({
                            list: ".file-upload-previews"
                        });
                    });
...
function socialShare(){
    ...
    if ( socialButtonsEnabled == 1 ){
        $('head').append( $('<link rel="stylesheet" type="text/css">').attr('href', 'http://localhost/devhucp/assets/css/jssocials.css') );
        $('head').append( $('<link rel="stylesheet" type="text/css">').attr('href', 'http://localhost/devhucp/assets/css/jssocials-theme-minima.css') );
        $.getScript( "http://localhost/devhucp/assets/js/jssocials.min.js", function( data, textStatus, jqxhr ) {
...
function initializeReadMore(){
    ...
    $.ajax({
        type: "GET",
        url: "http://localhost/devhucp/assets/js/readmore.min.js",
