function loadColor(action, newColor){
    $.ajax({
        url: "http://localhost/devhucp/assets/misc/customizer.php",
        method: "POST",
        cache: false,
        data: { action: action, new_color: newColor, default_color: defaultColor },
        success: function(results){
            if( action == "change_color" ){
                $('head').append( $('<link rel="stylesheet" type="text/css">').attr('href', 'http://localhost/devhucp/assets/css/temporary-style.css') );
                $('link').each(function() {
...
function createCustomizer(defaultColor) {
    ...
    $.getScript( "http://localhost/devhucp/assets/js/icheck.min.js", function( data, textStatus, jqxhr ) {
        if ($("input[type=checkbox]").length > 0) {
            ...
            $('.customizer #checkbox-rtl').on('ifChecked', function(event){
                $('head').append( $('<link rel="stylesheet" type="text/css">').attr('href', 'http://localhost/devhucp/assets/css/rtl.css') );
                $("body").addClass("rtl");
                $(".owl-carousel").trigger("destroy.owl.carousel");
                initializeOwl();
            });
            $('.customizer #checkbox-rtl').on('ifUnchecked', function(event){
                $('link[href="http://localhost/devhucp/assets/css/rtl.css"]').remove();
    ...
    $('head').append( $('<link rel="stylesheet" type="text/css">').attr('href', 'http://localhost/devhucp/assets/misc/spectrum.css') );
    $.getScript( "http://localhost/devhucp/assets/misc/spectrum.js", function( data, textStatus, jqxhr ) {
